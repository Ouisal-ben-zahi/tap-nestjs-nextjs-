...
def get_cv_from_minio(candidate_id: int, use_original_only: bool = False) -> Tuple[bool, Optional[str], Optional[bytes]]:
    """
    Récupère le CV depuis Supabase Storage (anciennement MinIO).
    Si use_original_only=True (ex: portfolio long), utilise uniquement le CV original (fichiers_versions.cv_ancienne_url).
    ...
    try:
        if supabase_db is None:
            print("⚠️ Supabase DB non configurée pour get_cv_from_minio")
            return False, None, None

        storage = get_supabase_storage()
        if not storage or not storage.client:
            print("⚠️ Supabase Storage non configuré pour get_cv_from_minio")
            return False, None, None
        ...
        try:
            if not use_original_only:
                ...
                row_corr = resp_corr.data[0] if resp_corr.data else None
                if row_corr and row_corr.get("corrected_pdf_minio_url"):
                    cv_path = row_corr.get("corrected_pdf_minio_url")
                    source = "corrected PDF"
        except Exception as e:
            print(f"⚠️ Erreur lecture corrected_pdf_minio_url dans fichiers_versions (Supabase): {e}")

        # 2. Utiliser le CV original si pas de corrigé ou use_original_only
        if not cv_path:
            try:
                resp_orig = (
                    supabase_db.table("fichiers_versions")
                ...
                row_orig = resp_orig.data[0] if resp_orig.data else None
            except Exception as e:
                print(f"⚠️ Erreur lecture cv_ancienne_url dans fichiers_versions (Supabase): {e}")
                row_orig = None

            if row_orig and row_orig.get("cv_ancienne_url"):
                cv_path = row_orig.get("cv_ancienne_url")
                source = "original"
        ...
        if not success or not file_bytes:
            print(f"❌ Erreur téléchargement CV depuis Supabase Storage: {error}")
            return False, None, None

        filename = os.path.basename(object_name)
        print(f"✅ CV récupéré depuis Supabase Storage ({source}): {filename}")
...
def _convert_html_to_pdf_impl(
    ...
    try:
        ...
        if pdf_page_format == "one-page":
            ...
            for base in [
                os.path.join(_root, "frontend", "src", "20260312_044613"),
                os.path.join(_backend, "..", "frontend", "src", "20260312_044613"),
                "/app/frontend/src/20260312_044613",
                "/frontend/src/20260312_044613",
                os.path.join(os.getcwd(), "frontend", "src", "20260312_044613"),
                os.path.join(os.getcwd(), "..", "frontend", "src", "20260312_044613"),
            ]:
        ...
        try:
            ...
            storage = get_supabase_storage()
            if not storage or not storage.client:
                raise RuntimeError("Supabase Storage non initialisé pour upload du PDF de portfolio")
            ...
            try:
                ...
                if del_ok:
                    print(f"🗑️ Ancien PDF supprimé du stockage: {object_name}")
            except Exception as del_err:
                print(f"⚠️  Erreur suppression ancien PDF dans le stockage: {del_err}")
