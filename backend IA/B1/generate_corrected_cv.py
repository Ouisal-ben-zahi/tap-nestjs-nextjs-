...
def prepare_prompt(analysis, cv_text: str | None = None, feedback_comments: str | None = None, lang: str = "fr"):
    ...
    if lang == "fr":
        ...
        instr = """
        You are a senior expert in recruitment and CV optimization.

        You receive:
        1) a candidate profile (JSON) with raw information
        2) the original CV text (non-optimized)
        3) feedback comments from the client (if available) to improve the CV

        GOAL:
        Analyze the CV critically and then REBUILD a professional CV:
        clear, coherent, and optimized for recruiters.

        ⚠️ STRICTLY FORBIDDEN:
        - Do not copy/paste sentences from the original CV
        - Do not reproduce descriptions word-for-word
        - Every description must be rephrased, improved and made more professional

        STEP 1 — ANALYSIS (internal, do not output this part):
        Analyze the CV and identify:
        - missing or unclear information
        - inconsistencies (dates, titles, levels)
        - weak formulations
        - strengths of the profile
        - possible improvements (clarity, impact, business value)

        STEP 2 — CORRECTION & OPTIMIZATION:
        From this analysis:
        - complete missing information when possible
        - rephrase all descriptions with a professional tone
        - highlight impact, skills and business value
        - use action verbs (developed, designed, optimized, automated, etc.)
        - remove repetitions and vague wording
        - adapt the content to a modern, easy-to-read CV

        OUTPUT FORMAT:
        You must return ONLY valid JSON
        strictly following this structure (field names in French, but ALL TEXT CONTENT in ENGLISH):
        ...
        - "" for empty strings, [] for empty lists
        - Max 12 skills
        - EXPERIENCES / REALISATIONS (adapt to profile):
          * If the candidate has NO experience or at most 2 experiences: max 4 realisations (projects); Experiences can be short or empty.
          * If the candidate has MORE than 2 experiences: max 4 experiences AND max 2 realisations.
        - For each realisation: fill "contexte" (context, e.g. academic project, internship, client engagement), "stack" (technologies, e.g. Python, React, SQL) and "detail" (result / description).
        - Experience description: max 3 bullet points, each max 6 words.
        - Summary (resume): max 280 characters.
        - Content MUST be written in NATURAL ENGLISH, professional, clear and value-oriented.
        - NO sentence copy-pasted from the original CV.
...
def transform_corrected_json_to_cv_context(corrected, lang: str = "fr"):
    """
    ...
    # Normaliser la langue pour le template (fr / en)
    lang = (lang or "fr").lower()
    if lang not in ("fr", "en"):
        lang = "fr"
...
def generate_corrected_cv(
    *,
    ...
    prompt = prepare_prompt(a1_payload, cv_text=cv_text, feedback_comments=feedback_comments, lang=lang)
    ...
    cv_context = transform_corrected_json_to_cv_context(corrected, lang=lang)
    ...
    if not (c.get("profile_image_url") or "").strip():
        raw_img = (
            orig.get("profile_image_url")
            or orig.get("image_minio_url")
        ...
        if raw_img and not (raw_img.startswith("http://") or raw_img.startswith("https://")):
            try:
                ...
                if storage and storage.client:
                    signed = storage.get_file_url(raw_img)
                    if signed:
                        raw_img = signed
            except Exception as e:
                print(f"⚠️ Impossible de générer une URL Supabase pour l'image CV '{raw_img}': {e}")
        c["profile_image_url"] = raw_img
...
def main():
    ...
    parser.add_argument("--template", required=True, help="chemin vers le template HTML (ex: frontend/src/20260312_044613/CV_template_A4.html)")
