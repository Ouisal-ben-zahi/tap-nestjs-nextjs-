...
def get_talent_card_from_minio(candidate_id: int) -> Tuple[bool, Optional[Dict], Optional[str]]:
    """
    Récupère la Talent Card (A1) depuis Supabase Storage (anciennement MinIO).
    ...
    try:
        if supabase_db is None:
            return False, None, "Supabase DB non configurée"
        storage = get_supabase_storage()
        if not storage or not storage.client:
            return False, None, "Supabase Storage non initialisé"

        # Récupérer candidate_uuid via fichiers_versions
        try:
            resp = (
            ...
            print(f"⚠️ Erreur lecture fichiers_versions pour Talent Card: {e}")
            row = None

        candidate_uuid = (row or {}).get("candidate_uuid")
        ...
        print(f"✅ Talent Card récupérée depuis Supabase Storage pour candidat {candidate_id}")
...
def get_corrected_cv_from_minio(candidate_id: int) -> Tuple[bool, Optional[Dict], Optional[str]]:
    """
    Récupère le CV corrigé (B1) depuis Supabase Storage.
    ...
    try:
        ...
        # Récupérer le chemin du JSON corrigé depuis fichiers_versions
        try:
            resp = (
                supabase_db.table("fichiers_versions")
                .select("corrected_json_minio_url")
                .eq("candidate_id", candidate_id)
                .limit(1)
                .execute()
            )
            ...
            print(f"⚠️ Erreur lecture fichiers_versions pour CV corrigé: {e}")
            row = None

        json_path = (row or {}).get("corrected_json_minio_url")
        if not json_path:
            print(f"⚠️ CV corrigé non trouvé pour candidat {candidate_id}")
            return False, None, None

        success, file_bytes, error = storage.download_file(json_path)
        if not success or not file_bytes:
            return False, None, error or "Erreur téléchargement CV corrigé"

        corrected_cv_data = json.loads(file_bytes.decode("utf-8"))
        print(f"✅ CV corrigé récupéré pour candidat {candidate_id} depuis Supabase Storage")
...
def get_portfolio_from_minio(candidate_id: int, candidate_uuid: str = None) -> Tuple[bool, Optional[Dict], Optional[str]]:
    """
    Récupère le portfolio (B2) depuis Supabase Storage.
    ...
    try:
        ...
        if not candidate_uuid:
            print("⚠️ candidate_uuid requis pour récupérer le portfolio depuis le stockage")
            return False, None, None
        ...
        print(f"✅ Portfolio récupéré pour candidat {candidate_id} depuis Supabase Storage")
...
def get_candidate_data_from_db(candidate_id: int) -> Tuple[bool, Optional[Dict], Optional[str]]:
    """
    Récupère les données de base du candidat depuis Supabase (table candidates).
    ...
    try:
        ...
        resp = (
            ...
            .select("id, prenom, nom, titre_profil, email, phone, annees_experience, ville, pays, linkedin")
            .eq("id", candidate_id)
            .limit(1)
            .execute()
        )
        ...
        print("✅ Données candidat récupérées depuis Supabase")
...
def ask_gemini(user_text, conversation_history):
    ...
    # Ne pas activer la logique "réponse trop courte" si l'utilisateur n'a encore rien dit
    # (ex: premier tour où user_text == "" pour générer la première question)
    if user_text_stripped and (is_test or len(user_text_stripped) < 10):
        # Si c'est un test ou une réponse trop courte, demander poliment de répéter
