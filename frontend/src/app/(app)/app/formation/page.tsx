    ...
    <div className="max-w-[1100px] mx-auto">
      <div className="relative mb-8 pb-8 border-b border-white/[0.04]">
        ...
        <div className="relative">
          ...
          <h1 className="text-[28px] sm:text-[36px] font-bold text-white tracking-[-0.04em] font-heading">Formations recommandées</h1>
          <p className="text-white/45 text-[14px] mt-2 font-light">Des modules courts et ciblés pour booster votre profil et combler vos lacunes.</p>
        </div>
      </div>
      ...
      {statsQuery.isLoading ? (
        ...
        <>
          <div className="bg-amber-500/[0.06] border border-amber-500/15 rounded-2xl p-6 mb-8 flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center shrink-0">
              <Lightbulb size={18} className="text-amber-500" />
            </div>
            <div>
              <h3 className="text-[14px] font-semibold text-white mb-1">Recommandations IA</h3>
              <p className="text-[13px] text-white/45 font-light">Ces formations ont été sélectionnées par notre IA en fonction de votre profil et du marché marocain.</p>
            </div>
          </div>

          <div className="flex items-center gap-3 mb-5">
            <div className="w-1 h-5 rounded-full bg-amber-500" />
            <h2 className="text-[13px] uppercase tracking-[2px] text-white/50 font-semibold">Modules disponibles</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {courses.map((course, i) => {
              ...
              return (
                <div key={i} className="group bg-zinc-900/50 border border-white/[0.06] rounded-xl p-5 hover:border-white/[0.12] transition-all duration-300 cursor-pointer">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[14px] font-semibold text-white mb-1.5 group-hover:text-tap-red transition-colors">{course.title}</h3>
                      <div className="flex flex-wrap items-center gap-2 text-[11px] text-white/40">
                        <span className="px-2 py-0.5 rounded-full bg-white/[0.05] border border-white/[0.08]">{course.category}</span>
                        <span className="flex items-center gap-1"><Clock size={10} />{course.duration}</span>
                        <span>{course.level}</span>
                      </div>
                    </div>
                    <ArrowRight size={14} className="text-white/0 group-hover:text-white/40 mt-1 transition-all shrink-0" />
                  </div>
                </div>
              );
            })}
