...
import { Users, MapPin, Briefcase, Clock, Sparkles, FileText, Upload } from "lucide-react";
      ...
      <div className="relative mb-8 pb-8 border-b border-white/[0.04]">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2.5 px-5 py-2.5 mb-4 rounded-full bg-emerald-500/[0.08] border border-emerald-500/15">
            ...
            <span className="text-[10px] uppercase tracking-[2.5px] text-emerald-500/80 font-semibold">Matching intelligent</span>
          </div>
          <h1 className="text-[28px] sm:text-[36px] font-bold text-white tracking-[-0.04em] font-heading">Offres qui vous correspondent</h1>
          <p className="text-white/45 text-[14px] mt-2 font-light">Notre IA analyse votre profil et vous connecte aux offres les plus pertinentes.</p>
        </div>
      </div>

      {!hasProfile && !statsQuery.isLoading ? (
        <EmptyState icon={<FileText className="w-12 h-12" />} title="Profil requis" description="Uploadez votre CV pour que l'IA puisse vous matcher avec les meilleures offres."
        ...
        <>
          <div className="bg-emerald-500/[0.06] border border-emerald-500/15 rounded-2xl p-6 mb-8 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-white/45 font-light">Notre algorithme analyse votre profil en continu et vous propose les offres les plus adaptées.</p>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="text-[13px] uppercase tracking-[2px] text-white/50 font-semibold">Vos candidatures</h2>
            </div>
            ...
            {appsQuery.isLoading ? (
              ...
              <EmptyState icon={<Briefcase className="w-10 h-10" />} title="Aucune candidature" description="Les offres matchées par l'IA apparaîtront ici." />
            ) : (
              <div className="space-y-3">
                {apps.map((app) => (
                  <div key={app.id} className="bg-zinc-900/50 border border-white/[0.06] rounded-xl p-5 hover:border-white/[0.1] transition">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-[15px] font-semibold text-white truncate">{app.job_title}</h3>
                          <span className={`text-[11px] px-2.5 py-1 rounded-full border font-medium shrink-0 ${statusBg(app.status)}`}>{app.status}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-3 text-[12px] text-white/40">
                          ...
                          {app.category && <span className="flex items-center gap-1"><MapPin size={12} />{app.category}</span>}
                          <span className="flex items-center gap-1"><Clock size={12} />{formatRelative(app.applied_at)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
